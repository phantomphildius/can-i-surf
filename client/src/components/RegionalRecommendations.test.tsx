...
describe('RegionalRecommendations', () => {
  ...
  describe('when the component is handed an error', () => {
    ...
    it('renders the returned error message', () => {
      ...
      expect(
        screen.getByText("Guess you're not going surfing today! Bummer.")
      ).toBeInTheDocument();
  ...
  describe('when the component is handed the response', () => {
    describe('without any data', () => {
      ...
      it('renders the empty state', () => {
        ...
        expect(header).toHaveTextContent('Best bets are non existent');

        expect(screen.getByRole('heading', { level: 3 })).toHaveTextContent(
          "Guess you're not going surfing today! Bummer."
        );
        expect(
          screen.getByText('Hopefully the drought ends soon')
        ).toBeInTheDocument();
      });
    });
